...
A public release package is planned for the project
repository~\cite{corti-package}.  It is intended to include the frozen C17 production
source and build instructions, the independent C17/Python validator, the
contiguous and sparse result files, the witness certificate, validation
reports, and the manuscript source.  At the time of this manuscript draft,
the repository and archival DOI are not yet public; the DOI in
reference~\cite{corti-package} is an explicit placeholder and must be replaced
by the resolved record before release.  The exact values reported in this
paper derive from the locally frozen campaign and validation artifacts.
...
C.~Corti,
\emph{Computational artifacts for OEIS A255885}, planned project release,
2026.
\url{https://github.com/carcorti/A255885}; archival record planned at
\url{https://doi.org/10.5281/zenodo.xxxxxxxx} (placeholder; replace with the
resolved DOI before public release).
